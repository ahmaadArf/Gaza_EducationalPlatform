...
@section('page-header')
    <!-- breadcrumb -->
    @section('PageTitle')
    {{ __('quiz.take_test') }}  :{{$current_question->quizze->name}}
    @stop
    <!-- breadcrumb -->
@endsection
@section('content')
        ...
        <div class="card card-statistics mb-30 ">
            <div class="card-body">
                <form method="POST" action="{{ route('student.dashboard.quiz.answer', ['quizze_id' => $quizze_id, 'question_index' => $question_index]) }}">
                    ...
                    @else
                    <button type="submit" onclick="alertEndExam()" class="col-3">{{trans('Students_trans.submit')}}</button>
                    @endif
...
<script>
    function alertEndExam() {
        alert(" {{ __('quiz.confirm_test_submission') }}");
    }
</script>
@endsection
